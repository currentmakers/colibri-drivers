...
static int32_t colors[] = {
    COLIBRI_COLOR_OK, COLIBRI_COLOR_OFF,
    COLIBRI_COLOR_WARNING, COLIBRI_COLOR_OFF,
    COLIBRI_COLOR_ERROR,COLIBRI_COLOR_OFF,
    COLIBRI_COLOR_INFO, COLIBRI_COLOR_OFF
};
...
static int tick(uint64_t time)
{
    ...
    {
        next = time + 200;
        host->set_rgb_color(colors[state]);
        state = (state + 1) % 8;
    }
    return state;
}
